...
import static dk.webbies.tajscheck.paser.AST.ObjectLiteral.*;
...
public class AstTransformer {
    ...
    public AstTransformer(ParseDeclaration.Environment environment) {
    ...
    public AstNode convert(ParseTree tree) {
        ...
        if (tree instanceof VariableStatementTree) {
            ...
            if (literal instanceof IdentifierToken) {
                ...
                try {
                    ...
                    if ((value.startsWith("\"") && value.endsWith("\"")) || (value.startsWith("'") && value.endsWith("'"))) {
                        ...
                        return new RegExpExpression(loc, value);
            ...
            List<Pair<Expression, Statement>> cases = switchStatement.caseClauses.stream().filter(clause -> clause instanceof CaseClauseTree).map(clause -> (CaseClauseTree) clause).map(this::convertCaseClause).collect(Collectors.toList());
            ...
            List<Expression> arguments = cast(Expression.class, array.elements.stream().map(this::convert).collect(Collectors.toList()));
            return new ArrayLiteral(loc, arguments);
    ...
    private Pair<Expression, Statement> convertCaseClause(CaseClauseTree caseClause) {
        ...
        return new Pair<>((Expression) convert(caseClause.expression), statement);
    }
    ...
    private static Operator convertOperator(Token operator) {
        switch (operator.type) {
            ...
            case MINUS_MINUS: return Operator.PRE_MINUS_MINUS;
            case PLUS_PLUS: return Operator.PRE_PLUS_PLUS;
